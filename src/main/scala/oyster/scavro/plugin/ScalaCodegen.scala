...
object scratch {
  ...
  case class Bar(str: String) extends Foo {
    type A = Int
    val aClass = classOf[A]
    def foo: A = 42
  }

  def foo[T <: Foo](t: T): T#A = {
    println(" -> " + t.str + " -> " + t.aClass)
    t.foo
  }
}
...
object ScalaCodegen {
  ...
  case class ScavroOption[T <: PrimativeScavroParam](name: String) extends ScavroParam
